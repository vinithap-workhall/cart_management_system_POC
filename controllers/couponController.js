...
export const applyCoupon = async (req,res,next) => {
  ...
  return res.status(403).json({ success: false, message: 'You do not have access to this cart' });
    ...
    if (cart.items.length === 0) {
      return res.status(400).json({ success: false, message: 'Cannot apply a coupon to an empty cart' });
    }
    ...
    const totals=await cartModel.couponRecal(cart);
...
export const removeCoupon = async (req,res,next) => {
  try {
    ...
    if(cart.couponCode === null){
      return res.status(404).json({ success: false, message: 'This card does not have couponCode' });
    }
    if (cart.status !== 'ACTIVE') {
      return res.status(400).json({ success: false, message: 'Cannot modify a checked-out cart' });
    }
